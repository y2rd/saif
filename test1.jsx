...
export default function AdminDashboard({
  ...
  const handleSaveCustomFont = (e) => {
    ...
    // تعيين الخط مباشرة للمتجر
    setStoreConfig(prev => ({ ...prev, fontFamily: newFontName.trim() }));
    ...
    alert(`تم رفع وتطبيق الخط "${newFontObj.name}" بنجاح!`);
  };
  ...
  const handleDeleteCustomFont = (fontId, fontName) => {
    if (window.confirm(`هل أنت متأكد من حذف الخط "${fontName}"؟`)) {
      ...
      if (storeConfig.fontFamily === fontName) {
        setStoreConfig(prev => ({ ...prev, fontFamily: 'DIN Next LT Arabic' }));
      }
    }
  };
  ...
  const handleSaveProductSubmit = (e) => {
    ...
    if (editingProduct) {
      const updatedProduct = {
        ...
        hasQuantityTiers: productForm.hasQuantityTiers,
        quantityTiers: formattedTiers
      };
  ...
  const handleApproveTopup = (topup) => {
    ...
    if (setTopupRequests) {
      setTopupRequests(prev => prev.map(t => t.id === topup.id ? { ...t, status: 'مقبول' } : t));
    }
  ...
  const handleRejectTopup = (topup) => {
    ...
    if (setTopupRequests) {
      setTopupRequests(prev => prev.map(t => t.id === topup.id ? { ...t, status: 'مرفوض', rejectReason: reason } : t));
    }
  ...
  return (
    <div className="admin-dashboard-container flex flex-col md:flex-row min-h-[calc(100vh-65px)] bg-[#F8FAFC] relative font-normal">
      ...
      {/* شريط الجوال المتجاوب: زر فتح الأقسام في اليمين ثابتاً */}
      <div className="md:hidden bg-white border-b border-gray-200 px-3 py-2 flex items-center justify-between sticky top-[53px] z-20 shadow-xs">
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => setMobileMenuOpen(true)}
            className="p-1.5 px-3 text-white bg-[#004956] hover:bg-[#00343D] rounded-xl text-xs font-semibold flex items-center gap-1.5 cursor-pointer transition shadow-2xs active:scale-95"
        ...
        <div className="flex items-center gap-1.5 text-[11px] text-gray-600 bg-gray-50 border border-gray-200 px-2.5 py-1 rounded-xl font-medium">
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
          <span>لوحة الإدارة</span>
        </div>
      </div>

      {/* شريط تبويبات أفقي سريع قابل للسحب والتحريك بسلاسة (Drag & Swipe) بنمط سلة */}
      <div 
        ref={tabsBarRef}
        ...
        className="md:hidden bg-white/95 backdrop-blur-xs border-b border-gray-100 px-2.5 py-2 overflow-x-auto flex items-center gap-1.5 scrollbar-none sticky top-[95px] z-10 select-none cursor-grab active:cursor-grabbing touch-pan-x"
      ...
      >
        {[
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-medium whitespace-nowrap flex items-center gap-1.5 transition cursor-pointer shrink-0 ${
              activeTab === tab.id 
                ? 'bg-[#004956] text-white shadow-xs font-semibold' 
                : 'bg-transparent text-gray-700 hover:text-black hover:bg-gray-100/50'
            }`}
          >
            <i className={`fa-solid ${tab.icon} text-[11px]`}></i>
            <span>{tab.label}</span>
            {tab.badge > 0 && (
              <span className="bg-amber-500 text-white rounded-full px-1 text-[9px] font-bold">
                {tab.badge}
              </span>
            )}
          </button>
              ...
              <div className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-200 space-y-3.5 shadow-2xs flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 pb-2 border-b border-gray-100">
                    ...
                    <h3 className="text-xs sm:text-sm font-bold text-gray-900">تصدير واستيراد ملف النسخة الاحتياطية (JSON)</h3>
                  </div>
              ...
              <div className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-200 space-y-3.5 shadow-2xs flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 pb-2 border-b border-gray-100">
                    ...
                    <h3 className="text-xs sm:text-sm font-bold text-gray-900">المزامنة السحابية المباشرة (Firebase Firestore)</h3>
                  </div>
                  <p className="text-[11px] text-gray-500 mt-2 leading-relaxed">
            ...
            <div className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-200 space-y-3 shadow-2xs">
              <div className="flex items-center justify-between border-b border-gray-100 pb-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xs sm:text-sm font-bold text-gray-900">خطوة مهمة: قواعد الحماية في Firebase (Firestore Rules)</h3>
                </div>
      ...
      {/* القائمة الجانبية (Sidebar) ثابتة باليمين: درج جانبي ثابت على الجوال، وقائمة جانبية أنيقة على الكمبيوتر */}
      <aside
        className={`bg-white border-l border-gray-200 flex flex-col justify-between shadow-lg md:shadow-xs transition-all duration-300 z-50 ${
          mobileMenuOpen
            ? 'fixed top-0 right-0 bottom-0 w-72 max-w-[85vw] h-full translate-x-0'
            : 'fixed top-0 right-0 bottom-0 w-72 max-w-[85vw] h-full translate-x-full md:static md:w-64 md:h-auto md:translate-x-0 md:flex'
        }`}
      >
        {/* رأس القائمة في الجوال مع زر إغلاق صريح */}
        <div className="md:hidden flex items-center justify-between p-3.5 border-b border-gray-100 bg-gray-50/80">
          <div className="flex items-center gap-2">
            <i className="fa-solid fa-layer-group text-emerald-800 text-sm"></i>
            <span className="text-xs font-bold text-gray-900">أقسام لوحة التحكم</span>
          </div>
          <button
            type="button"
            onClick={() => setMobileMenuOpen(false)}
            className="w-7 h-7 rounded-full bg-white border border-gray-200 flex items-center justify-center text-gray-500 text-xs hover:bg-gray-100 cursor-pointer"
          >
            ✕
          </button>
        </div>
        <div>
          <div className={`p-2.5 sm:p-3 space-y-4`}>
            ...
            <div>
              <div className="px-3 pb-1.5 text-[10px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-wider">
                الرئيسية والمبيعات
              </div>
              <div className="space-y-1">
                <button
                  onClick={() => { setActiveTab('analytics'); setMobileMenuOpen(false); }}
                  className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs sm:text-[13px] font-medium transition cursor-pointer ${
                    activeTab === 'analytics' ? 'bg-[#004956] text-white shadow-xs font-semibold' : 'text-gray-700 hover:bg-gray-100/80'
                  }`}
                >
                  <i className={`fa-solid fa-chart-line text-sm ${activeTab === 'analytics' ? 'text-white' : 'text-black'}`}></i>
                  <span>التحليلات والمؤشرات</span>
                </button>

                <button
                  onClick={() => { setActiveTab('orders'); setMobileMenuOpen(false); }}
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs sm:text-[13px] font-medium transition cursor-pointer ${
                    activeTab === 'orders' ? 'bg-[#004956] text-white shadow-xs font-semibold' : 'text-gray-700 hover:bg-gray-100/80'
                  }`}
                >
                  <div className="flex items-center gap-2.5">
                    <i className={`fa-solid fa-bag-shopping text-sm ${activeTab === 'orders' ? 'text-white' : 'text-black'}`}></i>
                  ...
                  {pendingOrdersCount > 0 ? (
                    ...
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${activeTab === 'orders' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-600'}`}>
                      {orders.length}
                    </span>
                  )}
                </button>
            ...
            <div>
              ...
              <div className="space-y-1">
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs sm:text-[13px] font-medium transition cursor-pointer ${
                    activeTab === 'products' ? 'bg-[#004956] text-white shadow-xs font-semibold' : 'text-gray-700 hover:bg-gray-100/80'
                  }`}
                >
                  <div className="flex items-center gap-2.5">
                    <i className={`fa-solid fa-boxes-stacked text-sm ${activeTab === 'products' ? 'text-white' : 'text-black'}`}></i>
                    <span>المنتجات والمخزون</span>
                  </div>
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${activeTab === 'products' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-600'}`}>
                    {products.length}
                  </span>
                </button>
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs sm:text-[13px] font-medium transition cursor-pointer ${
                    activeTab === 'categories' ? 'bg-[#004956] text-white shadow-xs font-semibold' : 'text-gray-700 hover:bg-gray-100/80'
                  }`}
                >
                  <div className="flex items-center gap-2.5">
                    <i className={`fa-solid fa-folder-tree text-sm ${activeTab === 'categories' ? 'text-white' : 'text-black'}`}></i>
                    <span>أقسام المتجر</span>
                  </div>
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${activeTab === 'categories' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-600'}`}>
                    {categories.length}
                  </span>
                </button>
              </div>
            ...
            <div>
              ...
              <div className="space-y-1">
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs sm:text-[13px] font-medium transition cursor-pointer ${
                    activeTab === 'customers' ? 'bg-[#004956] text-white shadow-xs font-semibold' : 'text-gray-700 hover:bg-gray-100/80'
                  }`}
                >
                  <div className="flex items-center gap-2.5">
                    <i className={`fa-solid fa-users text-sm ${activeTab === 'customers' ? 'text-white' : 'text-black'}`}></i>
                    <span>قاعدة العملاء</span>
                  </div>
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${activeTab === 'customers' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-600'}`}>
                    {customers.length}
                  </span>
                </button>
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs sm:text-[13px] font-medium transition cursor-pointer ${
                    activeTab === 'coupons' ? 'bg-[#004956] text-white shadow-xs font-semibold' : 'text-gray-700 hover:bg-gray-100/80'
                  }`}
                >
                  <div className="flex items-center gap-2.5">
                    <i className={`fa-solid fa-tags text-sm ${activeTab === 'coupons' ? 'text-white' : 'text-black'}`}></i>
                    <span>كوبونات الخصم</span>
                  </div>
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${activeTab === 'coupons' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-600'}`}>
                    {coupons.length}
                  </span>
                </button>
              </div>
            ...
            <div>
              ...
              <div className="space-y-1">
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs sm:text-[13px] font-medium transition cursor-pointer ${
                    activeTab === 'announcements' ? 'bg-[#004956] text-white shadow-xs font-semibold' : 'text-gray-700 hover:bg-gray-100/80'
                  }`}
                >
                  <div className="flex items-center gap-2.5">
                    <i className={`fa-solid fa-bullhorn text-sm ${activeTab === 'announcements' ? 'text-white' : 'text-black'}`}></i>
                    <span>أشرطة الإعلانات</span>
                  </div>
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${activeTab === 'announcements' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-600'}`}>
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs sm:text-[13px] font-medium transition cursor-pointer ${
                    activeTab === 'fonts' ? 'bg-[#004956] text-white shadow-xs font-semibold' : 'text-gray-700 hover:bg-gray-100/80'
                  }`}
                >
                  <i className={`fa-solid fa-palette text-sm ${activeTab === 'fonts' ? 'text-white' : 'text-black'}`}></i>
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs sm:text-[13px] font-medium transition cursor-pointer ${
                    activeTab === 'features' ? 'bg-[#004956] text-white shadow-xs font-semibold' : 'text-gray-700 hover:bg-gray-100/80'
                  }`}
                >
                  <div className="flex items-center gap-2.5">
                    <i className={`fa-solid fa-bolt text-sm ${activeTab === 'features' ? 'text-white' : 'text-black'}`}></i>
                    <span>مميزات المنتج (تسليم فوري، ضمان...)</span>
                  </div>
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs sm:text-[13px] font-medium transition cursor-pointer ${
                    activeTab === 'payments' ? 'bg-[#004956] text-white shadow-xs font-semibold' : 'text-gray-700 hover:bg-gray-100/80'
                  }`}
                >
                  <i className={`fa-solid fa-credit-card text-sm ${activeTab === 'payments' ? 'text-white' : 'text-black'}`}></i>
                  <span>وسائل الدفع والباركود</span>
                </button>

                <button
                  onClick={() => { setActiveTab('cloud-backup'); setMobileMenuOpen(false); }}
                  className={`w-full flex items-center justify-between px-3 py-1.5 md:py-2 rounded-xl text-xs sm:text-[13px] font-medium transition cursor-pointer ${
                    activeTab === 'cloud-backup' ? 'bg-[#004956] text-white shadow-xs font-semibold' : 'text-gray-700 hover:bg-gray-100/80'
                  }`}
                >
                  <div className="flex items-center gap-2.5">
                    <i className={`fa-solid fa-cloud-arrow-up text-sm ${activeTab === 'cloud-backup' ? 'text-white' : 'text-sky-600'}`}></i>
                    <span className="font-bold">النسخ الاحتياطي والسحابة</span>
                  </div>
      ...
      <main className="flex-1 p-2 sm:p-5 md:p-8 overflow-y-auto w-full min-w-0 transition-all">
                  ...
                  {topDemandedProducts.map((p, idx) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        {/* صورة المنتج المصغرة */}
                        <div className="w-11 h-11 rounded-xl bg-gray-100 border border-gray-200/80 overflow-hidden shrink-0 flex items-center justify-center">
                          {p.imageUrl ? (
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between gap-4">
              ...
              <button
                ...
                className="px-4 py-2 bg-[#004956] text-white text-xs font-bold rounded-xl shadow-xs hover:opacity-95 flex items-center gap-1.5 cursor-pointer transition-all active:scale-95 shrink-0"
              >
                  ...
                  <div className="bg-white p-12 text-center rounded-2xl border border-gray-200/80 shadow-2xs space-y-3">
                    ...
                    <div className="pt-2 flex items-center justify-center gap-2">
                      ...
                      <button
                        type="button"
                        onClick={handleOpenNewProduct}
                        className="px-4 py-2 bg-[#004956] text-white rounded-xl text-xs font-semibold cursor-pointer hover:opacity-95 transition flex items-center gap-1.5"
                      >
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-right text-xs">
                        <thead className="bg-[#F9FAFB] border-b border-gray-200 text-gray-500 font-semibold text-[11px]">
                          <tr>
                            <th className="py-3.5 px-4 font-semibold text-gray-600">المنتج</th>
                            <th className="py-3.5 px-3 font-semibold text-gray-600">نوع المنتج</th>
                            <th className="py-3.5 px-3 font-semibold text-gray-600">السعر</th>
                            <th className="py-3.5 px-3 font-semibold text-gray-600">المخزون</th>
                            <th className="py-3.5 px-4 font-semibold text-gray-600 text-center">إجراءات</th>
                          </tr>
                        </thead>
                            ...
                            return (
                              <tr key={p.id} className="hover:bg-gray-50/80 transition-colors group">
                                ...
                                <td className="py-3 px-4 max-w-[280px]">
                                  <div className="flex items-center gap-2.5">
                                    <div className="relative w-10 h-10 rounded-xl bg-gray-50 border border-gray-200/90 overflow-hidden shrink-0 flex items-center justify-center p-0.5">
                                      <img
                                        src={p.imageUrl}
                                        alt=""
              ...
              <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-2xs">
                <div className="overflow-x-auto">
                  <table className="w-full text-right text-xs">
                    <thead className="bg-[#F9FAFB] border-b border-gray-200 text-gray-500 font-semibold text-[11px]">
                      <tr>
                        <th className="py-3.5 px-4 font-semibold text-gray-600">رقم الطلب</th>
                        <th className="py-3.5 px-4 font-semibold text-gray-600">اسم العميل</th>
                        <th className="py-3.5 px-3 font-semibold text-gray-600">طريقة الدفع</th>
                        <th className="py-3.5 px-3 font-semibold text-gray-600">المبلغ</th>
                        <th className="py-3.5 px-3 font-semibold text-gray-600">إيصال التحويل</th>
                        <th className="py-3.5 px-3 font-semibold text-gray-600">حالة الطلب</th>
                        <th className="py-3.5 px-4 font-semibold text-gray-600 text-center">تحديث الحالة</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100/90">
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-end gap-2 flex-wrap">
              <button
                ...
                className="px-3.5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-xs flex items-center gap-2 cursor-pointer transition-all active:scale-95"
              >
              ...
              <button
                ...
                className="px-4 py-2.5 bg-[#004956] text-white text-xs font-bold rounded-xl shadow-xs hover:opacity-95 flex items-center gap-2 cursor-pointer transition-all active:scale-95"
              >
                <i className="fa-solid fa-user-plus text-xs"></i>
                <span>إضافة عميل / مشرف</span>
              </button>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-2xs">
              <div className="p-3 bg-gradient-to-r from-emerald-50 to-teal-50/50 border-b border-gray-100 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-xs sm:text-sm font-bold text-gray-900">طلبات شحن المحفظة من العملاء</h3>
                          ...
                          <tr key={c.id} className="hover:bg-gray-50/70 transition-colors">
                            ...
                            <td className="py-2.5 px-2 sm:px-4 text-center whitespace-nowrap">
                              <div className="inline-flex items-center justify-center gap-1 sm:gap-2">
                                ...
                                >
                                  <i className={`fa-solid ${isBlocked ? 'fa-lock-open' : 'fa-ban'} text-xs sm:text-sm`}></i>
                                </button>
            ...
            {showAddCustomerModal && (
              <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center z-50 p-4">
                <div className="bg-[#F9FAFB] rounded-3xl border-0 max-w-xl w-full p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            ...
            {selectedCustomerForView && (
              <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center z-50 p-4">
                <div className="bg-[#F9FAFB] rounded-3xl border-0 max-w-md w-full p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-200">
            ...
            {walletModalCustomer && (
              <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-in fade-in duration-150">
                <div className="bg-white rounded-3xl border border-gray-100 max-w-md w-full p-6 shadow-2xl space-y-5 animate-in zoom-in-95 duration-200 text-right" dir="rtl">
                  ...
                  <div className="flex items-center justify-between pb-3 border-b border-gray-100">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <h3 className="font-bold text-gray-900 text-sm sm:text-base">إضافة وتعديل رصيد المحفظة</h3>
            ...
            {showBroadcastModal && (
              <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
                <div className="bg-white rounded-3xl border border-gray-100 max-w-md w-full p-6 shadow-2xl space-y-4 text-right" dir="rtl">
                  <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                    <div className="flex items-center gap-2">
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-end">
              <button
                ...
                className="px-4 py-2.5 bg-[#004956] text-white text-xs font-bold rounded-xl shadow-xs hover:opacity-95 flex items-center gap-2 cursor-pointer transition-all active:scale-95"
              >
                <i className={`fa-solid ${showAddCouponModal ? 'fa-xmark' : 'fa-plus'} text-xs`}></i>
                <span>{showAddCouponModal ? 'إلغاء النافذة' : 'إنشاء كوبون جديد'}</span>
              </button>
            </div>
                          ...
                          <tr key={cp.id} className="hover:bg-gray-50/70 transition group">
                            ...
                            <td className="py-2 px-1.5 sm:px-2 text-center whitespace-nowrap">
                              <div className="inline-flex items-center justify-center gap-1.5 sm:gap-2">
                                ...
                                >
                                  <i className={`fa-solid ${cp.status === 'نشط' ? 'fa-pause' : 'fa-play'} text-xs sm:text-sm`}></i>
                                </button>
        ...
        {activeTab === 'categories' && (
          <div className="space-y-4">
            {/* شريط علوي: إحصائية وزر إضافة قسم جديد في أقصى اليسار */}
            ...
            {showAddCategoryModal && (
              <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
                <div className="bg-[#F9FAFB] rounded-2xl max-w-lg w-full p-5 shadow-2xl space-y-4 border-0 animate-in fade-in zoom-in-95 duration-200" dir="rtl">
                  <div className="flex items-center justify-between pb-3">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1.5 text-xs">
                          ...
                          <div className="w-9 h-9 rounded-xl bg-gray-50 border border-gray-200 overflow-hidden flex items-center justify-center p-1 shadow-2xs">
                            {newCatImage ? (
                ...
                const renderCategoryRow = (cat, isSub = false, parentName = '') => {
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 sm:p-4 hover:bg-gray-50/70 transition flex items-center justify-between gap-3 ${
                        isSub ? 'bg-gray-50/40 pr-8 sm:pr-12 border-r-4 border-[#004956]' : ''
                      }`}
                    >
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                        ...
                        {isSub && (
                          <div className="text-gray-400 text-xs sm:text-sm pl-1 flex items-center shrink-0" title="قسم فرعي">
                            <i className="fa-solid fa-arrow-turn-down-left -rotate-90"></i>
                          </div>
                        )}

                        {cat.imageUrl ? (
                          <div className={`${isSub ? 'w-9 h-9 sm:w-10 sm:h-10' : 'w-11 h-11'} rounded-xl bg-gray-50 border border-gray-200/80 p-1 flex items-center justify-center shrink-0 shadow-2xs overflow-hidden`}>
                            <img src={cat.imageUrl} alt={cat.name} className="w-full h-full object-contain" />
                          </div>
                        ) : cat.icon ? (
                          <div className={`${isSub ? 'w-9 h-9 sm:w-10 sm:h-10' : 'w-11 h-11'} rounded-xl bg-gray-50 border border-gray-200/80 flex items-center justify-center text-sm sm:text-base text-[#004956] shrink-0 shadow-2xs`}>
                            <i className={cat.icon}></i>
                          </div>
                        ) : (
                          <div className={`${isSub ? 'w-9 h-9 sm:w-10 sm:h-10' : 'w-11 h-11'} rounded-xl bg-gray-100 border border-gray-200/80 flex items-center justify-center text-sm sm:text-base text-gray-500 shrink-0`}>
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            <h4 className={`font-bold text-gray-800 ${isSub ? 'text-xs sm:text-[13px]' : 'text-xs sm:text-sm'} truncate`}>
                              {cat.name}
                            </h4>
                            {isSub ? (
                              <span className="px-1.5 py-0.5 rounded-md bg-[#004956]/10 text-[#004956] text-[9.5px] font-bold">
                                فرعي من: {parentName}
                              </span>
                            ) : cat.name !== 'الكل' && (
                              <span className="px-1.5 py-0.5 rounded-md bg-gray-100 text-gray-600 text-[9.5px] font-semibold">
                                قسم رئيسي
                              </span>
                            )}
                          </div>
                      ...
                      {/* الجانب الأيسر: إضافة فرعي + تحرير + حذف */}
                      <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
                        {/* زر إضافة قسم فرعي سريع للأقسام الرئيسية */}
                        {!isSub && cat.name !== 'الكل' && (
                          <button
                            type="button"
                            onClick={() => {
                            ...
                            className="px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg text-[#004956] hover:bg-[#004956]/10 text-[10px] sm:text-xs font-bold transition cursor-pointer flex items-center gap-1 border border-[#004956]/20"
                            title={`إضافة قسم فرعي تابع لـ ${cat.name}`}
                          >
                            <i className="fa-solid fa-plus text-[9px]"></i>
                            <span>قسم فرعي</span>
                          </button>
            ...
            {editingCatId && (
              <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
                <div className="bg-[#F9FAFB] rounded-2xl max-w-lg w-full p-5 shadow-2xl space-y-4 border-0" dir="rtl">
                  <div className="flex items-center justify-between pb-3">
                    <h3 className="font-bold text-sm text-gray-800 flex items-center gap-2">
        ...
        {activeTab === 'announcements' && (
          <div className="space-y-6">
            <div className="flex items-center justify-end">
              ...
              >
                <i className="fa-solid fa-plus text-xs"></i>
                <span>إضافة شريط إعلاني جديد</span>
              </button>
            </div>
        ...
        {activeTab === 'store-design' && (
          <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 sm:p-5 rounded-2xl border border-gray-200 shadow-2xs">
              <div>
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-xl bg-[#004956] text-white flex items-center justify-center text-sm shadow-xs">
                    <i className="fa-solid fa-brush"></i>
                  </div>
                  <div>
                    <h2 className="text-sm sm:text-base font-bold text-gray-900 flex items-center gap-2">
                      <span>تصميم وتخصيص واجهة المتجر</span>
                      <span className="text-[10px] bg-emerald-100 text-emerald-800 font-bold px-2 py-0.5 rounded-full">
                        نمط سلة الرسمي (Salla Style)
                      </span>
                    </h2>
                    <p className="text-[11px] text-gray-500 mt-0.5">
                      تحكم كامل في هوية المتجر، اسم وشعار المتجر، البانر الترحيبي، خطوط وثيمات سلة، وتنسيق الأقسام
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  type="button"
                  onClick={() => alert('تم تطبيق وحفظ تصميم المتجر بنجاح!')}
                  className="px-4 py-2 bg-[#004956] hover:bg-[#00343D] text-white text-xs font-bold rounded-xl shadow-xs transition flex items-center gap-1.5 cursor-pointer active:scale-98"
                >
                  <i className="fa-solid fa-check text-xs"></i>
              ...
              <div className="lg:col-span-8 space-y-5">
                ...
                <div className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-200 space-y-4 shadow-2xs">
                  <div className="flex items-center gap-2 border-b border-gray-100 pb-2.5">
                    ...
                    <h3 className="text-xs sm:text-sm font-bold text-gray-900">1. هوية ومعلومات المتجر الأساسية</h3>
                  </div>
                  ...
                  <div className="space-y-3.5">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-2.5">
                        ...
                        <label
                          className="w-9 h-9 bg-black hover:bg-gray-800 text-white rounded-xl flex items-center justify-center cursor-pointer shadow-xs active:scale-95 transition shrink-0"
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          {storeConfig.logoUrl ? (
                            ...
                            <div className="w-9 h-9 rounded-xl border border-dashed border-gray-300 bg-gray-50 flex items-center justify-center text-gray-400" title="لا يوجد لوقو مرفوع حالياً">
                ...
                {/* 2. ثيمات وألوان المتجر (بنظام ثيمات سلة الجاهزة ومخصص) */}
                <div className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-200 space-y-4 shadow-2xs">
                  <div className="flex items-center justify-between border-b border-gray-100 pb-2.5">
                    <div className="flex items-center gap-2">
                      <i className="fa-solid fa-palette text-black text-sm"></i>
                      <h3 className="text-xs sm:text-sm font-bold text-gray-900">2. ألوان وهوية الثيم</h3>
                    </div>
                    <span className="text-[10px] text-gray-500">اختر من ثيمات سلة أو خصص بنفسك</span>
                  </div>

                  {/* ثيمات سلة الجاهزة بنقرة واحدة */}
                  <div className="space-y-1.5">
                    <span className="text-[10px] font-bold text-gray-500 block">ثيمات سلة الجاهزة المعتمدة:</span>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      {[
                        { name: 'سلة الأصلي (زمردي)', primary: '#004956', accent: '#76e5d0', banner: '#00343D' },
                        { name: 'كلاسيك راقي (أسود)', primary: '#111827', accent: '#374151', banner: '#000000' },
                        { name: 'كحلي ملكي', primary: '#0f172a', accent: '#38bdf8', banner: '#1e293b' },
                        { name: 'عنابي فخم', primary: '#4a0404', accent: '#f87171', banner: '#2b0202' },
                        { name: 'بنفسجي تقني', primary: '#4c1d95', accent: '#c084fc', banner: '#2e1065' },
                        { name: 'أخضر عشبي', primary: '#065f46', accent: '#34d399', banner: '#064e3b' }
                      ].map((preset, idx) => (
                        <button
                          key={idx}
                          type="button"
                          ...
                          className={`p-2 rounded-xl border text-right transition cursor-pointer flex items-center justify-between ${
                            storeConfig.primaryColor === preset.primary
                              ? 'border-[#004956] bg-emerald-50/40 ring-1 ring-[#004956]'
                              : 'border-gray-200 bg-gray-50/50 hover:bg-white'
                          }`}
                        >
                          <div className="flex items-center gap-1.5 min-w-0">
                            <span className="w-3.5 h-3.5 rounded-full shrink-0 shadow-2xs" style={{ backgroundColor: preset.primary }}></span>
                            <span className="text-[10px] font-bold text-gray-800 truncate">{preset.name}</span>
                          </div>
                          {storeConfig.primaryColor === preset.primary && (
                            <span className="text-[#004956] text-xs font-bold shrink-0">✓</span>
                          )}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
                    <div className="p-2.5 bg-gray-50 rounded-xl border border-gray-200">
                      <label className="block text-[10px] font-bold text-gray-700 mb-1">اللون الأساسي</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="color"
                          value={storeConfig.primaryColor || '#004956'}
                        ...
                        <input
                          ...
                          value={storeConfig.primaryColor || '#004956'}
                          onChange={(e) => setStoreConfig(prev => ({ ...prev, primaryColor: e.target.value }))}
                          className="w-full p-1.5 bg-white border border-gray-200 rounded-lg text-xs font-mono font-bold"
                        />
                      </div>
                    </div>

                    <div className="p-2.5 bg-gray-50 rounded-xl border border-gray-200">
                      <label className="block text-[10px] font-bold text-gray-700 mb-1">اللون التمييزي</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="color"
                          value={storeConfig.accentColor || '#76e5d0'}
                    ...
                    <div className="p-2.5 bg-gray-50 rounded-xl border border-gray-200">
                      <label className="block text-[10px] font-bold text-gray-700 mb-1">لون البانر العلوي</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="color"
                          value={storeConfig.bannerBgColor || '#00343D'}
                ...
                {/* 3. الخطوط ونوع الخط الرسمي */}
                <div className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-200 space-y-4 shadow-2xs">
                  <div className="flex items-center justify-between border-b border-gray-100 pb-2.5">
                    <div className="flex items-center gap-2">
                      <i className="fa-solid fa-font text-black text-sm"></i>
                      <h3 className="text-xs sm:text-sm font-bold text-gray-900">3. خط المتجر (Typography)</h3>
                    </div>
                    <span className="text-[10px] text-emerald-800 font-bold bg-emerald-50 px-2 py-0.5 rounded">
                      الخط المعتمد: {storeConfig.fontFamily}
                    </span>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {[
                      { id: 'DIN Next LT Arabic', name: 'DIN Next LT Arabic', desc: 'خط منصة سلة الافتراضي الرسمي ⭐', badge: 'سلة الافتراضي' },
                      { id: 'Tajawal', name: 'Tajawal (تجوال)', desc: 'خط هادئ وأنيق جداً للنصوص والأسعار', badge: 'ثيمات سلة' },
                      { id: 'Cairo', name: 'Cairo (كايرو)', desc: 'خط عصري وواضح جداً للشاشات', badge: 'عصري' },
                      { id: 'Almarai', name: 'Almarai (المراعي)', desc: 'خط رسمي متوازن ومتناسق', badge: 'رسمي' }
                    ].map((f) => {
                      const isSel = storeConfig.fontFamily === f.id;
                      return (
                        <div
                          key={f.id}
                          onClick={() => setStoreConfig(prev => ({ ...prev, fontFamily: f.id }))}
                          className={`p-2.5 rounded-xl border text-right cursor-pointer transition flex items-center justify-between ${
                            isSel ? 'border-black bg-gray-50 ring-1 ring-black' : 'border-gray-200 bg-white hover:bg-gray-50/60'
                          }`}
                        >
                          <div>
                            <div className="flex items-center gap-1.5">
                              <span className="font-bold text-xs text-black">{f.name}</span>
                              <span className="text-[9px] px-1.5 py-0.2 rounded font-bold border border-gray-200 text-gray-500">
                                {f.badge}
                              </span>
                            </div>
                            <span className="text-[10px] text-gray-400 block mt-0.5">{f.desc}</span>
                          </div>
                          {isSel && <span className="text-black font-bold text-xs">✓</span>}
                        </div>
                      );
                    })}
                  </div>
                ...
                {/* 4. البانر الترويجي والبانرات الإعلانية */}
                <div className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-200 space-y-4 shadow-2xs">
                  <div className="flex items-center gap-2 border-b border-gray-100 pb-2.5">
                    <i className="fa-solid fa-rectangle-ad text-black text-sm"></i>
                    <h3 className="text-xs sm:text-sm font-bold text-gray-900">4. نصوص البانر الترحيبي والفوتر</h3>
                  </div>
                ...
                {/* 5. عناصر ومكونات الصفحة الرئيسية (محرر صفحات سلة المتقدم) */}
                <div className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-200 space-y-4 shadow-2xs">
                  <div className="flex items-center justify-between border-b border-gray-100 pb-2.5">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-lg bg-emerald-50 text-emerald-800 flex items-center justify-center text-xs">
                        <i className="fa-solid fa-shapes"></i>
                      </div>
                      <div>
                        <h3 className="text-xs sm:text-sm font-bold text-gray-900">5. عناصر الصفحة الرئيسية (بنظام سلة)</h3>
                        <p className="text-[10px] text-gray-400">تحكم في تشغيل وتخصيص كل عنصر في الواجهة الرئيسية للمتجر</p>
                      </div>
                    </div>
                    <span className="text-[10px] bg-emerald-100 text-emerald-800 font-bold px-2 py-0.5 rounded-full">
                      5 عناصر متاحة
                  ...
                  <div className="space-y-4">
                    {/* العنصر الأول: بانر متحرك (سلايدر العروض) */}
                    <div className="p-3.5 sm:p-4 rounded-xl border border-gray-200 bg-gray-50/50 space-y-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="w-7 h-7 rounded-lg bg-purple-50 text-purple-700 flex items-center justify-center text-xs">
                            <i className="fa-solid fa-images"></i>
                          </span>
                          <div>
                            <span className="text-xs font-bold text-gray-900 block">بانر (سلايدر متحرك)</span>
                            <span className="text-[10px] text-gray-400">بانر تفاعلي أعلى الصفحة مع رفع صورة ورابط توجيه</span>
                          </div>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            type="checkbox"
                            checked={storeConfig.homeSections?.bannerSlider?.enabled !== false}
                            onChange={(e) => {
                              ...
                              setStoreConfig(prev => ({
                                ...
                                homeSections: {
                                  ...prev.homeSections,
                                  bannerSlider: { ...(prev.homeSections?.bannerSlider || {}), enabled: checked }
                                }
                              }));
                            }}
                      ...
                      {storeConfig.homeSections?.bannerSlider?.enabled !== false && (
                        <div className="pt-3 border-t border-gray-200/60 space-y-3">
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                            <div>
                              <label className="block text-[10px] font-bold text-gray-700 mb-1">
                                ...
                                شارة البانر (الراديو / النص الترويجي)
                              </label>
                              <input
                                type="text"
                                value={storeConfig.homeSections?.bannerSlider?.slides?.[0]?.badgeText ?? 'عرض مميز وحصري'}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  setStoreConfig(prev => {
                                    const slides = [...(prev.homeSections?.bannerSlider?.slides || [])];
                                    if (!slides[0]) slides[0] = {};
                                    slides[0] = { ...slides[0], badgeText: val };
                                    return { ...prev, homeSections: { ...prev.homeSections, bannerSlider: { ...prev.homeSections?.bannerSlider, slides } } };
                                  });
                                }}
                                placeholder="مثال: عرض مميز وحصري (أو اتركه فارغاً للإخفاء)"
                                className="w-full p-2 bg-white border border-gray-200 rounded-lg text-xs outline-none focus:border-black font-semibold text-emerald-700"
                              />
                            </div>
                            <div>
                              <label className="block text-[10px] font-bold text-gray-700 mb-1">عنوان البانر الرئيسي</label>
                              <input
                                type="text"
                                value={storeConfig.homeSections?.bannerSlider?.slides?.[0]?.title || ''}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  setStoreConfig(prev => {
                                    const slides = [...(prev.homeSections?.bannerSlider?.slides || [])];
                                ...
                                placeholder="مثال: عروض حصرية لمزرعتك في Hay Day"
                                className="w-full p-2 bg-white border border-gray-200 rounded-lg text-xs outline-none focus:border-black"
                              />
                            </div>
                            <div>
                              <label className="block text-[10px] font-bold text-gray-700 mb-1">نص زر الإجراء</label>
                              <input
                                type="text"
                                value={storeConfig.homeSections?.bannerSlider?.slides?.[0]?.buttonText || ''}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  setStoreConfig(prev => {
                                    const slides = [...(prev.homeSections?.bannerSlider?.slides || [])];
                                    if (!slides[0]) slides[0] = {};
                                    slides[0] = { ...slides[0], buttonText: val };
                                    return { ...prev, homeSections: { ...prev.homeSections, bannerSlider: { ...prev.homeSections?.bannerSlider, slides } } };
                                  });
                                }}
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                            <div className="sm:col-span-2">
                              <label className="block text-[10px] font-bold text-gray-700 mb-1">الوصف الفرعي للبانر</label>
                              <input
                                type="text"
                                value={storeConfig.homeSections?.bannerSlider?.slides?.[0]?.subtitle || ''}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  setStoreConfig(prev => {
                                    const slides = [...(prev.homeSections?.bannerSlider?.slides || [])];
                                ...
                                placeholder="خصومات تصل إلى 30% على الصكوك ومواد التوسعة"
                                className="w-full p-2 bg-white border border-gray-200 rounded-lg text-xs outline-none focus:border-black"
                              />
                            </div>
                            <div>
                              <div className="flex items-center justify-between mb-1">
                                <label className="text-[10px] font-bold text-gray-700">
                                  <i className="fa-solid fa-border-top-left text-xs ml-1 text-gray-500"></i>
                                  استدارة الحدود (Border Radius)
                                </label>
                                <span className="text-[10px] font-mono text-gray-500 font-bold">
                                  {storeConfig.homeSections?.bannerSlider?.borderRadius ?? 16}px
                                </span>
                              </div>
                              <div className="flex items-center gap-1.5">
                                <input
                                  ...
                                  min="0"
                                  max="60"
                                  value={storeConfig.homeSections?.bannerSlider?.borderRadius ?? 16}
                                  onChange={(e) => {
                                    ...
                                    setStoreConfig(prev => ({
                                      ...
                                      homeSections: {
                                        ...prev.homeSections,
                                        bannerSlider: {
                                          ...prev.homeSections?.bannerSlider,
                                          borderRadius: val
                                        }
                                      }
                                    }));
                                  ...
                                  placeholder="16"
                                  className="w-20 p-2 bg-white border border-gray-200 rounded-lg text-xs font-mono font-bold text-center outline-none focus:border-black"
                                />
                                <div className="flex-1 flex items-center gap-1">
                                  {[0, 8, 16, 24, 32].map((r) => (
                                    <button
                                      ...
                                      className={`flex-1 py-1.5 text-[9px] font-bold rounded-md border transition cursor-pointer text-center ${
                                        (storeConfig.homeSections?.bannerSlider?.borderRadius ?? 16) === r
                          ...
                          {/* رفع صورة البانر ورابط التوجيه */}
                          <div className="bg-white p-2.5 rounded-xl border border-gray-200 space-y-2.5">
                            <div className="flex items-center justify-between">
                              <label className="text-[10px] font-bold text-gray-700 flex items-center gap-1.5">
                                ...
                                <span>صورة البانر</span>
                              </label>
                              <div className="flex items-center gap-2">
                                <label className="w-7 h-7 rounded-lg bg-black text-white hover:bg-gray-800 flex items-center justify-center cursor-pointer transition shadow-2xs" title="رفع صورة من جهازك">
                                      ...
                                      if (file) {
                                        ...
                                        reader.onload = (re) => {
                                          ...
                                          setStoreConfig(prev => {
                                            const slides = [...(prev.homeSections?.bannerSlider?.slides || [])];
                                            if (!slides[0]) slides[0] = {};
                                            slides[0] = { ...slides[0], imageUrl: b64 };
                                            return { ...prev, homeSections: { ...prev.homeSections, bannerSlider: { ...prev.homeSections?.bannerSlider, slides } } };
                                          });
                                        };
                                        reader.readAsDataURL(file);
                                ...
                                {storeConfig.homeSections?.bannerSlider?.slides?.[0]?.imageUrl && (
                                  <button
                                    type="button"
                                    onClick={() => {
                                      setStoreConfig(prev => {
                                        const slides = [...(prev.homeSections?.bannerSlider?.slides || [])];
                                        if (slides[0]) slides[0] = { ...slides[0], imageUrl: '' };
                                        return { ...prev, homeSections: { ...prev.homeSections, bannerSlider: { ...prev.homeSections?.bannerSlider, slides } } };
                                      });
                                    }}
                                    className="w-7 h-7 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 flex items-center justify-center text-xs transition cursor-pointer"
                                    title="حذف الصورة"
                                  >
                                    <i className="fa-solid fa-trash-can"></i>
                                  </button>
                            ...
                            <div className="flex items-center gap-2">
                              {storeConfig.homeSections?.bannerSlider?.slides?.[0]?.imageUrl ? (
                                <img
                                  src={storeConfig.homeSections?.bannerSlider?.slides?.[0]?.imageUrl}
                                  alt="معاينة"
                                  className="w-16 h-10 rounded-lg object-cover border border-gray-200 shrink-0"
                                />
                              ) : (
                                <div className="w-16 h-10 rounded-lg border border-dashed border-gray-300 bg-gray-50 flex items-center justify-center text-gray-400 shrink-0 text-xs">
                                  <i className="fa-solid fa-image"></i>
                                </div>
                              )}
                              <input
                                type="text"
                                value={storeConfig.homeSections?.bannerSlider?.slides?.[0]?.imageUrl || ''}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  setStoreConfig(prev => {
                                    const slides = [...(prev.homeSections?.bannerSlider?.slides || [])];
                                    if (!slides[0]) slides[0] = {};
                                    slides[0] = { ...slides[0], imageUrl: val };
                                    return { ...prev, homeSections: { ...prev.homeSections, bannerSlider: { ...prev.homeSections?.bannerSlider, slides } } };
                                  });
                                }}
                                placeholder="أو أدخل رابط صورة مباشر (URL)..."
                                className="flex-1 p-2 bg-gray-50 border border-gray-200 rounded-lg text-xs font-mono text-left outline-none focus:bg-white"
                              />
                            </div>

                            {/* رابط التوجيه عند الضغط على صورة البانر */}
                            <div className="pt-2 border-t border-gray-100 grid grid-cols-1 sm:grid-cols-2 gap-2">
                              <div>
                                <label className="block text-[10px] font-bold text-gray-700 mb-1">
                                  <i className="fa-solid fa-link text-xs ml-1 text-gray-400"></i>
                                  رابط التوجيه عند الضغط (URL أو خارجي)
                                </label>
                                <input
                                  type="text"
                                  value={storeConfig.homeSections?.bannerSlider?.slides?.[0]?.linkUrl || ''}
                                  onChange={(e) => {
                                    const val = e.target.value;
                                    setStoreConfig(prev => {
                                      const slides = [...(prev.homeSections?.bannerSlider?.slides || [])];
                                      if (!slides[0]) slides[0] = {};
                                      slides[0] = { ...slides[0], linkUrl: val };
                                      return { ...prev, homeSections: { ...prev.homeSections, bannerSlider: { ...prev.homeSections?.bannerSlider, slides } } };
                                    });
                                  }}
                                  placeholder="https://example.com أو رابط صفحة..."
                                  className="w-full p-2 bg-gray-50 border border-gray-200 rounded-lg text-xs font-mono text-left outline-none focus:bg-white"
                                />
                              </div>
                              <div>
                                <label className="block text-[10px] font-bold text-gray-700 mb-1">
                                  <i className="fa-solid fa-tag text-xs ml-1 text-gray-400"></i>
                                  أو توجيه لتصنيف محدد
                                </label>
                                <select
                                  value={storeConfig.homeSections?.bannerSlider?.slides?.[0]?.linkCat || ''}
                                  onChange={(e) => {
                                    const val = e.target.value;
                                    setStoreConfig(prev => {
                                      const slides = [...(prev.homeSections?.bannerSlider?.slides || [])];
                                      if (!slides[0]) slides[0] = {};
                                      slides[0] = { ...slides[0], linkCat: val };
                                      return { ...prev, homeSections: { ...prev.homeSections, bannerSlider: { ...prev.homeSections?.bannerSlider, slides } } };
                                    });
                                  }}
                                  className="w-full p-2 bg-gray-50 border border-gray-200 rounded-lg text-xs font-bold text-gray-800 outline-none focus:bg-white"
                                >
                                  <option value="">بدون تصنيف (توجيه للرابط أعلاه)</option>
                                  {categories.map((c, i) => (
                    ...
                    {/* العنصر الثاني: صور مربعة */}
                    <div className="p-3.5 sm:p-4 rounded-xl border border-gray-200 bg-gray-50/50 space-y-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="w-7 h-7 rounded-lg bg-blue-50 text-blue-700 flex items-center justify-center text-xs">
                            <i className="fa-solid fa-border-all"></i>
                          </span>
                          <div>
                            <span className="text-xs font-bold text-gray-900 block">صور مربعة (كروت العروض)</span>
                            <span className="text-[10px] text-gray-400">تحكم بالصور، الروابط والتصنيفات الموجهة لكل كارت</span>
                          </div>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            type="checkbox"
                            checked={storeConfig.homeSections?.squareImages?.enabled !== false}
                            onChange={(e) => {
                              const checked = e.target.checked;
                              setStoreConfig(prev => ({
                                ...prev,
                                homeSections: {
                                  ...prev.homeSections,
                                  squareImages: { ...(prev.homeSections?.squareImages || {}), enabled: checked }
                                }
                              }));
                            }}
                            className="sr-only peer"
                      ...
                      {storeConfig.homeSections?.squareImages?.enabled !== false && (
                        <div className="pt-3 border-t border-gray-200/60 space-y-3">
                          <div>
                            <label className="block text-[10px] font-bold text-gray-700 mb-1">عنوان قسم الصور المربعة</label>
                            <input
                              type="text"
                              value={storeConfig.homeSections?.squareImages?.title || ''}
                              onChange={(e) => {
                                const val = e.target.value;
                                setStoreConfig(prev => ({
                                  ...prev,
                                  homeSections: {
                                    ...prev.homeSections,
                                    squareImages: { ...prev.homeSections?.squareImages, title: val }
                                  }
                                }));
                              }}
                              className="w-full p-2 bg-white border border-gray-200 rounded-lg text-xs outline-none focus:border-black"
                              placeholder="تسوق حسب الفئات المميزة"
                            />
                          </div>

                          <div className="space-y-2.5">
                            <span className="text-[10px] font-bold text-gray-700 block">إدارة الكروت المربعة (رفع صور وتعيين روابط):</span>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              {(storeConfig.homeSections?.squareImages?.items || []).map((item, idx) => (
                                <div key={item.id || idx} className="bg-white p-3 rounded-xl border border-gray-200 space-y-2">
                                  <div className="flex items-center gap-2.5">
                                    ...
                                    <div className="flex-1 min-w-0 space-y-1">
                                      <div className="flex items-center justify-between">
                                        <span className="text-[10px] font-bold text-gray-800">كارت #{idx + 1}</span>
                                        <label className="w-6 h-6 rounded-md bg-black text-white hover:bg-gray-800 flex items-center justify-center cursor-pointer transition shadow-2xs" title="رفع صورة للكارت">
                                            ...
                                            onChange={(e) => {
                                              ...
                                              if (file) {
                                                ...
                                                reader.onload = (re) => {
                                                  const b64 = re.target.result;
                                                  setStoreConfig(prev => {
                                                    const items = [...(prev.homeSections?.squareImages?.items || [])];
                                                    items[idx] = { ...items[idx], imageUrl: b64 };
                                                    return {
                                                      ...prev,
                                                      homeSections: {
                                                        ...prev.homeSections,
                                                        squareImages: { ...prev.homeSections?.squareImages, items }
                                                      }
                                                    };
                                                  });
                                                };
                                      ...
                                      <input
                                        ...
                                        className="w-full p-1.5 bg-gray-50 border border-gray-200 rounded text-[11px] font-bold outline-none focus:bg-white"
                                      />
                                    </div>
                                  </div>
                                  ...
                                  <div className="grid grid-cols-2 gap-1.5 pt-1 border-t border-gray-100">
                                    <div>
                                      <label className="block text-[9px] text-gray-500 font-bold mb-0.5">رابط مخصص (URL)</label>
                                      <input
                                        type="text"
                                        value={item.linkUrl || ''}
                                        onChange={(e) => {
                                          ...
                                          setStoreConfig(prev => {
                                            const items = [...(prev.homeSections?.squareImages?.items || [])];
                                            items[idx] = { ...items[idx], linkUrl: val };
                                            return {
                                              ...prev,
                                              homeSections: {
                                                ...prev.homeSections,
                                        ...
                                        placeholder="https://..."
                                        className="w-full p-1.5 bg-gray-50 border border-gray-200 rounded text-[10px] font-mono text-left outline-none focus:bg-white"
                                      />
                                    </div>
                                    <div>
                                      <label className="block text-[9px] text-gray-500 font-bold mb-0.5">أو ربط بتصنيف</label>
                                      <select
                                        value={item.linkCat || ''}
                                        onChange={(e) => {
                                          const val = e.target.value;
                                          setStoreConfig(prev => {
                                            const items = [...(prev.homeSections?.squareImages?.items || [])];
                                            items[idx] = { ...items[idx], linkCat: val };
                                            return {
                                              ...prev,
                                              homeSections: {
                                                ...prev.homeSections,
                                        ...
                                        className="w-full p-1.5 bg-gray-50 border border-gray-200 rounded text-[10px] font-bold outline-none focus:bg-white"
                                      >
                    ...
                    {/* العنصر الثالث: منتجات متحركة (اختيار تصنيف كامل أو منتجات محددة) */}
                    <div className="p-3.5 sm:p-4 rounded-xl border border-gray-200 bg-gray-50/50 space-y-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="w-7 h-7 rounded-lg bg-amber-50 text-amber-700 flex items-center justify-center text-xs">
                            <i className="fa-solid fa-arrows-left-right-to-line"></i>
                          </span>
                          <div>
                            <span className="text-xs font-bold text-gray-900 block">منتجات متحركة</span>
                            <span className="text-[10px] text-gray-400">سلايدر منتجات بنمط سلة مع تحديد مصدر المنتجات بدقة</span>
                          </div>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            type="checkbox"
                            checked={storeConfig.homeSections?.movingProducts?.enabled !== false}
                            onChange={(e) => {
                              const checked = e.target.checked;
                              setStoreConfig(prev => ({
                                ...prev,
                                homeSections: {
                                  ...prev.homeSections,
                                  movingProducts: { ...(prev.homeSections?.movingProducts || {}), enabled: checked }
                                }
                              }));
                            }}
                            className="sr-only peer"
                      ...
                      {storeConfig.homeSections?.movingProducts?.enabled !== false && (
                        <div className="pt-3 border-t border-gray-200/60 space-y-3">
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            <div>
                              <label className="block text-[10px] font-bold text-gray-700 mb-1">عنوان قسم المنتجات المتحركة</label>
                              <input
                                type="text"
                                value={storeConfig.homeSections?.movingProducts?.title || ''}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  setStoreConfig(prev => ({
                                    ...prev,
                                    homeSections: {
                                      ...prev.homeSections,
                                      movingProducts: { ...prev.homeSections?.movingProducts, title: val }
                                    }
                                  }));
                                }}
                                className="w-full p-2 bg-white border border-gray-200 rounded-lg text-xs outline-none focus:border-black"
                            ...
                            <div>
                              <label className="block text-[10px] font-bold text-gray-700 mb-1">الوصف الفرعي</label>
                              <input
                                type="text"
                                value={storeConfig.homeSections?.movingProducts?.subtitle || ''}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  setStoreConfig(prev => ({
                                    ...prev,
                                    homeSections: {
                                      ...prev.homeSections,
                                      movingProducts: { ...prev.homeSections?.movingProducts, subtitle: val }
                                    }
                                  }));
                                }}
                                className="w-full p-2 bg-white border border-gray-200 rounded-lg text-xs outline-none focus:border-black"
                          ...
                          {/* خيارات تحديد مصدر المنتجات المتحركة: كل المنتجات / تصنيف كامل / منتجات محددة */}
                          <div className="bg-white p-3 rounded-xl border border-gray-200 space-y-2.5">
                            <label className="block text-[10px] font-bold text-gray-800">
                              <i className="fa-solid fa-filter text-amber-600 ml-1"></i>
                              مصدر المنتجات في السلايدر المتحرك:
                            </label>

                            <div className="grid grid-cols-3 gap-2">
                              {[
                                { id: 'all', label: 'كل المنتجات', icon: 'fa-globe' },
                                { id: 'category', label: 'تصنيف كامل', icon: 'fa-tags' },
                                { id: 'custom', label: 'منتجات محددة', icon: 'fa-check-double' }
                              ].map((src) => {
                                const activeSource = storeConfig.homeSections?.movingProducts?.sourceType || 'all';
                                  ...
                                  <button
                                    ...
                                    onClick={() => {
                                      setStoreConfig(prev => ({
                                        ...
                                        homeSections: {
                                          ...prev.homeSections,
                                          movingProducts: { ...prev.homeSections?.movingProducts, sourceType: src.id }
                                        }
                                      }));
                                    }}
                            ...
                            {/* في حال اختيار تصنيف كامل */}
                            {(storeConfig.homeSections?.movingProducts?.sourceType === 'category') && (
                              <div className="pt-2 border-t border-gray-100">
                                <label className="block text-[10px] font-bold text-gray-700 mb-1">اختر التصنيف لعرض منتجاته:</label>
                                <select
                                  value={storeConfig.homeSections?.movingProducts?.selectedCategory || ''}
                                  onChange={(e) => {
                                    const val = e.target.value;
                                    setStoreConfig(prev => ({
                                      ...prev,
                                      homeSections: {
                                        ...prev.homeSections,
                                        movingProducts: { ...prev.homeSections?.movingProducts, selectedCategory: val }
                                      }
                                    }));
                                  }}
                                  className="w-full p-2 bg-gray-50 border border-gray-200 rounded-lg text-xs font-bold text-gray-800 outline-none focus:bg-white"
                                >
                                  <option value="">-- اختر التصنيف المراد --</option>
                                  {categories.map((c, i) => (
                                    <option key={i} value={c.name}>{c.name}</option>
                                  ))}
                                </select>
                            ...
                            {/* في حال اختيار منتجات محددة يدوياً */}
                            {(storeConfig.homeSections?.movingProducts?.sourceType === 'custom') && (
                              <div className="pt-2 border-t border-gray-100 space-y-2">
                                <div className="flex items-center justify-between">
                                  <label className="text-[10px] font-bold text-gray-700">حدد المنتجات التي تريد إظهارها في السلايدر:</label>
                                  <span className="text-[10px] text-emerald-800 font-bold bg-emerald-50 px-2 py-0.5 rounded-full">
                                    {((storeConfig.homeSections?.movingProducts?.selectedProductIds) || []).length} منتج محدد
                                  </span>
                                </div>
                                <div className="max-h-48 overflow-y-auto space-y-1.5 pr-1 border border-gray-100 rounded-lg p-1.5 bg-gray-50/50">
                                  {products.map((p) => {
                                    const selectedIds = storeConfig.homeSections?.movingProducts?.selectedProductIds || [];
                                    ...
                                    return (
                                      <div
                                        ...
                                        onClick={() => {
                                          setStoreConfig(prev => {
                                            const current = prev.homeSections?.movingProducts?.selectedProductIds || [];
                                            const nextIds = current.includes(p.id)
                                            ...
                                            return {
                                              ...
                                              homeSections: {
                                                ...prev.homeSections,
                                                movingProducts: {
                                                  ...prev.homeSections?.movingProducts,
                                        ...
                                        className={`p-2 rounded-lg border flex items-center justify-between cursor-pointer transition ${
                                          isSelected ? 'border-emerald-600 bg-emerald-50/60' : 'border-gray-200 bg-white hover:bg-gray-50'
                                        }`}
                                      >
                                        <div className="flex items-center gap-2 min-w-0">
                                          ...
                                          <div className="min-w-0">
                                            ...
                                            <span className="text-[10px] text-gray-500 block truncate">{p.category}</span>
                                          </div>
                                        </div>
                                        <div className="shrink-0 mr-2">
                                          <input
                                            ...
                                            checked={isSelected}
                                            readOnly
                                            className="w-4 h-4 text-[#004956] rounded border-gray-300 focus:ring-0 cursor-pointer pointer-events-none"
                                          />
                                        </div>
                                      </div>
                                    );
                    ...
                    {/* العنصر الرابع: قائمة عناصر */}
                    <div className="p-3.5 sm:p-4 rounded-xl border border-gray-200 bg-gray-50/50 space-y-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="w-7 h-7 rounded-lg bg-teal-50 text-teal-700 flex items-center justify-center text-xs">
                            <i className="fa-solid fa-list-check"></i>
                          </span>
                          <div>
                            <span className="text-xs font-bold text-gray-900 block">قائمة عناصر وتصنيفات</span>
                            <span className="text-[10px] text-gray-400">عرض تصنيفات وأقسام المتجر كشريط أزرار تفاعلية</span>
                          </div>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            type="checkbox"
                            checked={storeConfig.homeSections?.itemsList?.enabled !== false}
                            onChange={(e) => {
                              const checked = e.target.checked;
                              setStoreConfig(prev => ({
                                ...prev,
                                homeSections: {
                                  ...prev.homeSections,
                                  itemsList: { ...(prev.homeSections?.itemsList || {}), enabled: checked }
                                }
                              }));
                            }}
                            className="sr-only peer"
                      ...
                      {storeConfig.homeSections?.itemsList?.enabled !== false && (
                        <div className="pt-3 border-t border-gray-200/60">
                          <label className="block text-[10px] font-bold text-gray-700 mb-1">عنوان قائمة العناصر</label>
                          <input
                            type="text"
                            value={storeConfig.homeSections?.itemsList?.title || ''}
                            onChange={(e) => {
                              const val = e.target.value;
                              setStoreConfig(prev => ({
                                ...prev,
                                homeSections: {
                                  ...prev.homeSections,
                                  itemsList: { ...prev.homeSections?.itemsList, title: val }
                                }
                              }));
                            }}
                            className="w-full p-2 bg-white border border-gray-200 rounded-lg text-xs outline-none focus:border-black"
                    ...
                    {/* العنصر الخامس: بانر عريض */}
                    <div className="p-3.5 sm:p-4 rounded-xl border border-gray-200 bg-gray-50/50 space-y-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="w-7 h-7 rounded-lg bg-rose-50 text-rose-700 flex items-center justify-center text-xs">
                            <i className="fa-solid fa-panorama"></i>
                          </span>
                          <div>
                            <span className="text-xs font-bold text-gray-900 block">بانر عريض</span>
                            <span className="text-[10px] text-gray-400">بانر ترويجي عريض كامل العرض مع رفع صورة ورابط</span>
                          </div>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            type="checkbox"
                            checked={storeConfig.homeSections?.wideBanner?.enabled !== false}
                            onChange={(e) => {
                              const checked = e.target.checked;
                              setStoreConfig(prev => ({
                                ...prev,
                                homeSections: {
                                  ...prev.homeSections,
                                  wideBanner: { ...(prev.homeSections?.wideBanner || {}), enabled: checked }
                                }
                              }));
                            }}
                            className="sr-only peer"
                      ...
                      {storeConfig.homeSections?.wideBanner?.enabled !== false && (
                        <div className="pt-3 border-t border-gray-200/60 space-y-3">
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            <div>
                              <label className="block text-[10px] font-bold text-gray-700 mb-1">عنوان البانر العريض</label>
                              <input
                                type="text"
                                value={storeConfig.homeSections?.wideBanner?.title || ''}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  setStoreConfig(prev => ({
                                    ...prev,
                                    homeSections: {
                                      ...prev.homeSections,
                                      wideBanner: { ...prev.homeSections?.wideBanner, title: val }
                                    }
                                  }));
                                }}
                                className="w-full p-2 bg-white border border-gray-200 rounded-lg text-xs outline-none focus:border-black"
                            ...
                            <div>
                              <label className="block text-[10px] font-bold text-gray-700 mb-1">نص زر الإجراء (CTA)</label>
                              <input
                                type="text"
                                value={storeConfig.homeSections?.wideBanner?.buttonText || ''}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  setStoreConfig(prev => ({
                                    ...prev,
                                    homeSections: {
                                      ...prev.homeSections,
                                      wideBanner: { ...prev.homeSections?.wideBanner, buttonText: val }
                                    }
                                  }));
                                }}
                                className="w-full p-2 bg-white border border-gray-200 rounded-lg text-xs outline-none focus:border-black"
                          ...
                          <div>
                            ...
                            <input
                              type="text"
                              value={storeConfig.homeSections?.wideBanner?.subtitle || ''}
                              onChange={(e) => {
                                const val = e.target.value;
                                setStoreConfig(prev => ({
                                  ...prev,
                                  homeSections: {
                                    ...prev.homeSections,
                                    wideBanner: { ...prev.homeSections?.wideBanner, subtitle: val }
                                  }
                                }));
                              }}
                              className="w-full p-2 bg-white border border-gray-200 rounded-lg text-xs outline-none focus:border-black"
                          ...
                          {/* رفع صورة البانر العريض والرابط */}
                          <div className="bg-white p-2.5 rounded-xl border border-gray-200 space-y-2.5">
                            <div className="flex items-center justify-between">
                              <label className="text-[10px] font-bold text-gray-700 flex items-center gap-1.5">
                                <i className="fa-solid fa-image text-rose-600"></i>
                                <span>صورة البانر العريض</span>
                              </label>
                              <div className="flex items-center gap-2">
                                <label className="w-7 h-7 rounded-lg bg-black text-white hover:bg-gray-800 flex items-center justify-center cursor-pointer transition shadow-2xs" title="رفع صورة من جهازك">
                                  <i className="fa-solid fa-arrow-up-from-bracket text-xs"></i>
                                      ...
                                      if (file) {
                                        ...
                                        reader.onload = (re) => {
                                          ...
                                          setStoreConfig(prev => ({
                                            ...
                                            homeSections: {
                                              ...prev.homeSections,
                                              wideBanner: { ...prev.homeSections?.wideBanner, imageUrl: b64 }
                                            }
                                          }));
                                        };
                                        reader.readAsDataURL(file);
                                ...
                                {storeConfig.homeSections?.wideBanner?.imageUrl && (
                                  <button
                                    type="button"
                                    onClick={() => {
                                      setStoreConfig(prev => ({
                                        ...
                                        homeSections: {
                                          ...prev.homeSections,
                                          wideBanner: { ...prev.homeSections?.wideBanner, imageUrl: '' }
                                        }
                                      }));
                                    }}
                                    className="w-7 h-7 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 flex items-center justify-center text-xs transition cursor-pointer"
                            ...
                            <div className="flex items-center gap-2">
                              {storeConfig.homeSections?.wideBanner?.imageUrl ? (
                                <img
                                  src={storeConfig.homeSections?.wideBanner?.imageUrl}
                                  alt="معاينة البانر العريض"
                                  className="w-16 h-10 rounded-lg object-cover border border-gray-200 shrink-0"
                                />
                              ) : (
                                <div className="w-16 h-10 rounded-lg border border-dashed border-gray-300 bg-gray-50 flex items-center justify-center text-gray-400 shrink-0 text-xs">
                              ...
                              <input
                                type="text"
                                value={storeConfig.homeSections?.wideBanner?.imageUrl || ''}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  setStoreConfig(prev => ({
                                    ...prev,
                                    homeSections: {
                                      ...prev.homeSections,
                                      wideBanner: { ...prev.homeSections?.wideBanner, imageUrl: val }
                                    }
                                  }));
                                }}
                                placeholder="أو أدخل رابط صورة مباشر (URL)..."
                            ...
                            <div className="pt-2 border-t border-gray-100 grid grid-cols-1 sm:grid-cols-2 gap-2">
                              <div>
                                ...
                                <input
                                  type="text"
                                  value={storeConfig.homeSections?.wideBanner?.linkUrl || ''}
                                  onChange={(e) => {
                                    const val = e.target.value;
                                    setStoreConfig(prev => ({
                                      ...prev,
                                      homeSections: {
                                        ...prev.homeSections,
                                        wideBanner: { ...prev.homeSections?.wideBanner, linkUrl: val }
                                      }
                                    }));
                                  }}
                                  placeholder="https://... أو رابط صفحة"
                                  className="w-full p-2 bg-gray-50 border border-gray-200 rounded-lg text-xs font-mono text-left outline-none focus:bg-white"
                                />
                              </div>
                              <div>
                                ...
                                <select
                                  value={storeConfig.homeSections?.wideBanner?.linkCat || ''}
                                  onChange={(e) => {
                                    const val = e.target.value;
                                    setStoreConfig(prev => ({
                                      ...prev,
                                      homeSections: {
                                        ...prev.homeSections,
                                        wideBanner: { ...prev.homeSections?.wideBanner, linkCat: val }
                                      }
                                    }));
                                  }}
                                  className="w-full p-2 bg-gray-50 border border-gray-200 rounded-lg text-xs font-bold text-gray-800 outline-none focus:bg-white"
                                >
                                  <option value="">بدون تصنيف (توجيه للرابط)</option>
                                  {categories.map((c, i) => (
                                    <option key={i} value={c.name}>{c.name}</option>
                                  ))}
                                </select>
                ...
                <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm">
                  <div className="flex items-center justify-between border-b border-gray-100 pb-2.5 mb-3">
                    <span className="text-xs font-bold text-gray-900 flex items-center gap-1.5">
                      ...
                      <span>معاينة حية لشكل المتجر</span>
                    </span>
                  ...
                  {/* إطار محاكي لشاشة الجوال */}
                  <div className="rounded-2xl border-2 border-gray-800 overflow-hidden bg-gray-50 shadow-md">
                    {/* شريط الإعلانات في المعاينة */}
                    <div className="py-1 px-2 text-center text-[9px] font-bold truncate text-white" style={{ backgroundColor: storeConfig.primaryColor }}>
                      ⚡ تسليم فوري لجميع الطلبات على مدار 24 ساعة
                    </div>

                    {/* هيدر المتجر في المعاينة */}
                    <div className="bg-white px-3 py-2 border-b border-gray-100 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        <span className="w-5 h-5 rounded-full bg-gray-100 flex items-center justify-center text-[8px]">🛒</span>
                      </div>
                    </div>

                    {/* البانر الترويجي في المعاينة */}
                    <div className="p-3 text-white text-center space-y-0.5" style={{ backgroundColor: storeConfig.bannerBgColor }}>
                      <h4 className="text-[11px] font-bold truncate">{storeConfig.bannerTitle}</h4>
                      <p className="text-[8px] opacity-80 line-clamp-1">{storeConfig.bannerDesc}</p>
                    </div>

                    {/* بطاقات أقسام مصغرة في المعاينة */}
                    <div className="p-2 space-y-1.5">
                      <span className="text-[9px] font-bold text-gray-700 block">الأقسام:</span>
                      <div className="grid grid-cols-3 gap-1 text-center">
                        {categories.slice(0, 3).map((c, i) => (
                          <div key={i} className="p-1.5 bg-white rounded-lg border border-gray-100 text-[8px] font-bold text-black truncate shadow-2xs">
                            {c.name}
                          </div>
                        ))}
                      </div>

                      {/* بطاقة منتج مصغرة في المعاينة */}
                      <span className="text-[9px] font-bold text-gray-700 block pt-1">أحدث المنتجات:</span>
                      <div className="bg-white p-1.5 rounded-xl border border-gray-100 shadow-2xs flex items-center gap-2">
                        <div className="w-10 h-10 rounded-lg bg-gray-100 overflow-hidden shrink-0 flex items-center justify-center">
                          <img src={products[0]?.imageUrl} alt="" className="w-full h-full object-cover" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <span className="block text-[9px] font-bold text-black truncate">{products[0]?.title}</span>
                          <span className="block text-[9px] font-black font-mono text-red-700">{formatPrice(products[0]?.price, activeCurrency)}</span>
                        </div>
                        <button
                          type="button"
                          className="px-2 py-1 text-white text-[8px] font-bold rounded-md shrink-0 shadow-2xs"
                          style={{ backgroundColor: storeConfig.primaryColor }}
                        >
                          أضف
                        </button>
                    ...
                    {/* فوتر المعاينة */}
                    <div className="p-2 text-center text-[7px] text-gray-400 bg-white border-t border-gray-100">
                      {storeConfig.footerCopyright}
                    </div>
                  </div>
        ...
        {activeTab === 'payments' && (
          <div className="max-w-3xl mx-auto bg-white rounded-2xl border border-gray-100 p-6 shadow-xs space-y-6">
            ...
            <div className="space-y-5">
              ...
              <div className="p-4 bg-gray-50 border border-gray-300 rounded-2xl space-y-3">
                <div className="flex items-center justify-between">
              ...
              {/* 5. سعر صرف الدولار مقابل الدينار العراقي */}
              ...
              <button
                onClick={() => showToast('تم حفظ وتحديث إعدادات الباركود ووسائل الدفع بنجاح')}
                className="w-full py-3 text-white text-xs font-bold rounded-xl shadow-xs transition cursor-pointer hover:opacity-95"
                style={{ backgroundColor: storeConfig.primaryColor }}
              >
                حفظ صور الباركود والبيانات
              </button>
            </div>
          </div>
        )}
        ...
        {activeTab === 'features' && (
          <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div key={feat.id || index} className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-200 shadow-2xs space-y-4">
                  <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-xl bg-gray-100 flex items-center justify-center text-gray-800">
                        {feat.customIconUrl ? (
                          <img src={feat.customIconUrl} alt="" className="w-5 h-5 object-contain" />
                        ) : (
                          <i className={`${feat.icon || 'fa-solid fa-bolt'} text-black text-sm`}></i>
                        )}
                      </div>
                      <span className="font-bold text-sm text-gray-900">الميزة رقم {index + 1}: {feat.title}</span>
                    </div>

                    {/* تفعيل / تعطيل هذه الميزة المحددة */}
                  ...
                  <div className="text-xs pt-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {feat.customIconUrl && (
                        <div className="relative w-9 h-9 rounded-xl border border-gray-200 p-1 flex items-center justify-center bg-gray-50 shrink-0">
            ...
            <div className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-200 shadow-2xs space-y-3">
              ...
              <div className="p-2 sm:p-4 bg-gray-50 rounded-2xl border border-gray-100 flex items-center justify-center">
                  ...
                  {(storeConfig.productFeatures?.items || []).filter(f => f.enabled !== false).map((feat, fIdx) => (
                    <div key={fIdx} className="py-2 px-1 sm:px-2 bg-white/80 backdrop-blur-md rounded-[5px] border border-black/[0.04] shadow-xs flex flex-col justify-center items-center">
                      {feat.customIconUrl ? (
                        ...
                        <i className={`${feat.icon || 'fa-solid fa-bolt'} text-black text-xs sm:text-sm block mb-1`}></i>
                      )}
            ...
            <button
              type="button"
              onClick={() => showToast('تم حفظ وتحديث إعدادات مميزات المنتج بنجاح')}
      ...
      {showProductModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs">
          <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 shadow-2xl border border-gray-100" dir="rtl">
            ...
            <form onSubmit={handleSaveProductSubmit} className="space-y-4 mt-4 text-xs">
              ...
              {/* السعر الأساسي وسعر التكلفة والتخفيض والكمية */}
                  ...
                  <div className="space-y-2 border border-gray-100 rounded-xl p-2.5 bg-gray-50/70">
                    {selectedOrderDetails.items.map((it, idx) => (
                      <div key={idx} className="p-2 bg-white rounded-lg border border-gray-200/80 flex items-start gap-2.5">
                        {it.imageUrl && (
                          <img src={it.imageUrl} alt="" className="w-10 h-10 rounded-md object-cover border border-gray-100 shrink-0" />
                        )}
