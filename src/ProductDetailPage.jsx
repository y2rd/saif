                    ...
                    >
                      {activeFeatures.map((feat, fIndex) => (
                        <div
                          ...
                          className="relative overflow-hidden p-2 sm:p-4 rounded-full aspect-square bg-white shadow-none hover:shadow-none transition-all duration-300 flex flex-col items-center justify-center text-center group min-w-0 w-full max-w-[150px] mx-auto border-0 outline-none ring-0"
                        >
                          {/* أيقونة سوداء مع حركة ناعمة عند التمرير بدون أي خلفية أو ظل */}
                          <div 
                            className="rounded-full bg-transparent flex items-center justify-center mb-1.5 sm:mb-2 transition-all duration-500 ease-out shrink-0 w-7 h-7 sm:w-10 sm:h-10 group-hover:-translate-y-1 shadow-none"
                            style={{ boxShadow: 'none' }}
                          >
                            {feat.customIconUrl ? (
                              <img src={feat.customIconUrl} alt="" className="object-contain brightness-0 w-3.5 h-3.5 sm:w-4 sm:h-4 transition-transform duration-500 ease-out group-hover:scale-110 group-hover:-rotate-3" />
                            ) : (
                              <i className={`${feat.icon || 'fa-solid fa-bolt'} text-black text-xs sm:text-base transition-transform duration-500 ease-out group-hover:scale-110 group-hover:-rotate-3`}></i>
                            )}
                          </div>
                          {/* نصوص الميزة */}
                          <h4 className="font-bold text-gray-900 text-[9.5px] sm:text-[11.5px] tracking-tight truncate w-full group-hover:text-black transition-colors mb-0.5 text-center px-1">
                            {feat.title}
                          </h4>
                          <p className="text-[8px] sm:text-[9.5px] text-gray-400 font-medium leading-tight line-clamp-1 w-full text-center px-1">
                            {feat.subtitle}
                          </p>
                        </div>
                      ))}
